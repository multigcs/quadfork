...
navMission_t *navLoadLeg(uint8_t leg) {
	...
	if (curLeg->type == NAV_LEG_DO_SET_ROI) {
		if (curLeg->targetLat != (double)0.0 && curLeg->targetLon != (double)0.0) {
			// set ROI
			gimbalData.roiLat = curLeg->targetLat;
			gimbalData.roiLon = curLeg->targetLon;
			gimbalData.roiAlt = curLeg->targetAlt;
			gimbalData.roiActive = 1;
		} else {
			gimbalData.roiActive = 0;
		}
		if (++leg < NAV_MAX_MISSION_LEGS && navData.missionLegs[leg].type > 0) {
			return navLoadLeg(leg);
		} else {
			return &navData.missionLegs[0];
		}
	}
...
void navNavigate(void) {
	...
	if ((int)p[NAV_HDFRE_CHAN] > 0 && (int)p[NAV_HDFRE_CHAN] <= RADIO_MAX_CHANNELS) {
		...
		if (!navData.homeActionFlag && ( navData.headFreeMode == NAV_HEADFREE_SETTING ||
			...
			if ((supervisorData.state & STATE_FLYING) && navData.homeLeg.targetLat != (double)0.0f && navData.homeLeg.targetLon != (double)0.0f) {
				if (NAV_HF_HOME_DIST_D_MIN && NAV_HF_HOME_DIST_FREQ
	...
	if (navData.mode > NAV_STATUS_MANUAL) {
		...
		if ((vertStick > p[CTRL_DBAND_THRO]  && !navData.setCeilingReached)  || vertStick < -p[CTRL_DBAND_THRO]) {
			...
			navData.targetHoldSpeedAlt = (vertStick - p[CTRL_DBAND_THRO] * (vertStick > 0.0f ? 1.0f : -1.0f)) * p[NAV_ALT_POS_OM] *
										 (1.0f / RADIO_MID_THROTTLE);
		...
		navData.targetHoldSpeedAlt = constrainFloat(navData.targetHoldSpeedAlt,
									 (navData.holdMaxVertSpeed < p[NAV_MAX_DECENT]) ? -navData.holdMaxVertSpeed : -p[NAV_MAX_DECENT], navData.holdMaxVertSpeed);
	...
	if (navData.mode == NAV_STATUS_MISSION) {
		float a, b, c;
		if (gimbalData.roiActive == 1) {
			gimbalData.roiHeading = navCalcBearing(gpsData.lat, gpsData.lon, gimbalData.roiLat, gimbalData.roiLon);
			a = navCalcDistance(gpsData.lat, gpsData.lon, gimbalData.roiLat, gimbalData.roiLon);
			b = ALTITUDE - (float)gimbalData.roiAlt;
		} else {
			gimbalData.roiHeading = 0.0;
			a = navData.holdDistance;
			b = ALTITUDE - curLeg->poiAltitude;
		}
		c = __sqrtf(a*a + b*b);
		gimbalData.roiAngle = asinf(a/c) * RAD_TO_DEG - 90.0f;
	} else {
		gimbalData.roiAngle = 0.0f;
		gimbalData.roiHeading = 0.0f;
	}
