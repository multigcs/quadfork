...
void supervisorCreateSOCTable(void) {
	...
	for (i = 0; i < SUPERVISOR_SOC_TABLE_SIZE+1; i++) {
		...
		supervisorData.socTable[i] = p[SPVR_BAT_CRV1] + p[SPVR_BAT_CRV2]*x + p[SPVR_BAT_CRV3]*x*x + p[SPVR_BAT_CRV4]*x*x*x +
									 p[SPVR_BAT_CRV5]*x*x*x*x + p[SPVR_BAT_CRV6]*x*x*x*x*x;
	}
}
...
void supervisorTaskCode(void *unused) {
	...
	while (1) {
		...
		if (supervisorData.state & STATE_CALIBRATION) {
			...
			if (RADIO_THROT < p[CTRL_MIN_THROT] && RADIO_RUDD > +500 && RADIO_FLAPS < -250 && !navData.homeActionFlag
		...
		if (supervisorData.state & STATE_FLYING) {
			...
			if (supervisorData.soc < 99.0f) {
				supervisorData.flightSecondsAvg += (supervisorData.flightTime / (100.0f - supervisorData.soc) - supervisorData.flightSecondsAvg) *
												   (0.01f / SUPERVISOR_RATE);
...
void supervisorInit(void) {
	...
	supervisorData.supervisorTask = CoCreateTask(supervisorTaskCode, (void *)0, SUPERVISOR_PRIORITY,
									&supervisorTaskStack[SUPERVISOR_STACK_SIZE-1], SUPERVISOR_STACK_SIZE);
}
