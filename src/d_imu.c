...
void dIMUInit(void) {
	...
	/*
		MPU6050_I2C_Init();
	    AQ_NOTICE("I2C: test...mpu-test\n");
		if (MPU6050_TestConnection() == 1) {
		    AQ_NOTICE("I2C: test...MPU6050 found\n");
		}
	    AQ_NOTICE("I2C: test...mpu-init\n");
		MPU6050_Initialize();
	    AQ_NOTICE("I2C: test...done\n");
	*/
	AQ_NOTICE("DIMU: preinit..\n");
#ifdef DIMU_HAVE_MPU6000
	mpu6000PreInit();
#endif
#ifdef DIMU_HAVE_MAX21100
	...
	ms5611PreInit();
#endif
	AQ_NOTICE("DIMU: preinit...ok\n");

#ifdef DIMU_HAVE_MPU6000
	AQ_NOTICE("DIMU: MPU6000 sensor init..\n");
	mpu6000Init();
	AQ_NOTICE("DIMU: MPU6000 sensor init...ok\n");
#endif
#ifdef DIMU_HAVE_MAX21100
	AQ_NOTICE("DIMU: MAX21100 sensor init..\n");
	max21100Init();
	AQ_NOTICE("DIMU: MAX21100 sensor init...ok\n");
#endif
#ifdef DIMU_HAVE_EEPROM
	AQ_NOTICE("DIMU: EEPROM init..\n");
	eepromInit();
	AQ_NOTICE("DIMU: EEPROM init...ok\n");
	//    dIMUWriteCalib();
	dIMUReadCalib();
#endif
#ifdef DIMU_HAVE_HMC5983
	AQ_NOTICE("DIMU: MAG sensor init..\n");
	if (hmc5983Init() == 0) {
		AQ_NOTICE("DIMU: MAG sensor init...failed!\n");
	} else {
		AQ_NOTICE("DIMU: MAG sensor init...ok\n");
	}
#endif
#ifdef DIMU_HAVE_MS5611
	AQ_NOTICE("DIMU: PRES sensor init..\n");
	if (ms5611Init() == 0) {
		AQ_NOTICE("DIMU: PRES sensor init...failed!\n");
	} else {
		AQ_NOTICE("DIMU: PRES sensor init...ok\n");
	}
#endif
