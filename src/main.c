...
void zero_ccm_section () __attribute__ ((section (".init")));
void zero_ccm_section () {
	extern char _sccm, _eccm;
	char *dst = &_sccm;
	while (dst < &_eccm) {
		*dst++ = 0;
	}
...
int main(void) {
	...
	zero_ccm_section(); // zero ccm, TODO: move it to startup-file(asm)
