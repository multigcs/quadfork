...
void TIM2_IRQHandler(void) {
	int8_t pwmPort;
#ifdef PWM_IRQ_TIM2_CH1
	if (TIM_GetITStatus(TIM2, TIM_IT_CC1) == SET) {
		pwmPort = PWM_IRQ_TIM2_CH1;
		TIM_ClearITPendingBit(TIM2, TIM_IT_CC1);
		pwmData[pwmPort].callback(TIM_GetCapture1(TIM2), GPIO_ReadInputDataBit((GPIO_TypeDef *)pwmPorts[pwmPort], (uint16_t)pwmPins[pwmPort]));
	}
#endif
#ifdef PWM_IRQ_TIM2_CH2
	if (TIM_GetITStatus(TIM2, TIM_IT_CC2) == SET) {
		pwmPort = PWM_IRQ_TIM2_CH2;
...
void TIM3_IRQHandler(void) {
	int8_t pwmPort;
#ifdef PWM_IRQ_TIM3_CH1
	if (TIM_GetITStatus(TIM3, TIM_IT_CC1) == SET) {
		pwmPort = PWM_IRQ_TIM3_CH1;
		TIM_ClearITPendingBit(TIM3, TIM_IT_CC1);
		pwmData[pwmPort].callback(TIM_GetCapture1(TIM3), GPIO_ReadInputDataBit((GPIO_TypeDef *)pwmPorts[pwmPort], (uint16_t)pwmPins[pwmPort]));
	}
#endif
#ifdef PWM_IRQ_TIM3_CH2
	if (TIM_GetITStatus(TIM3, TIM_IT_CC2) == SET) {
		pwmPort = PWM_IRQ_TIM3_CH2;
		TIM_ClearITPendingBit(TIM3, TIM_IT_CC2);
		pwmData[pwmPort].callback(TIM_GetCapture3(TIM3), GPIO_ReadInputDataBit((GPIO_TypeDef *)pwmPorts[pwmPort], (uint16_t)pwmPins[pwmPort]));
	}
#endif
#ifdef PWM_IRQ_TIM3_CH3
	if (TIM_GetITStatus(TIM3, TIM_IT_CC3) == SET) {
		pwmPort = PWM_IRQ_TIM3_CH3;
		TIM_ClearITPendingBit(TIM3, TIM_IT_CC3);
		pwmData[pwmPort].callback(TIM_GetCapture3(TIM3), GPIO_ReadInputDataBit((GPIO_TypeDef *)pwmPorts[pwmPort], (uint16_t)pwmPins[pwmPort]));
	}
#endif
#ifdef PWM_IRQ_TIM3_CH4
	if (TIM_GetITStatus(TIM3, TIM_IT_CC4) == SET) {
		pwmPort = PWM_IRQ_TIM3_CH4;
		TIM_ClearITPendingBit(TIM3, TIM_IT_CC4);
		pwmData[pwmPort].callback(TIM_GetCapture4(TIM3), GPIO_ReadInputDataBit((GPIO_TypeDef *)pwmPorts[pwmPort], (uint16_t)pwmPins[pwmPort]));
	}
#endif
}
