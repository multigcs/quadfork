...
static void mpu6000CalibAcc(float *in, volatile float *out) {
	...
	// bias
	a = -(in[0] + p[IMU_ACC_BIAS_X] + p[IMU_ACC_BIAS1_X]*dImuData.dTemp + p[IMU_ACC_BIAS2_X]*dImuData.dTemp2 +
		  p[IMU_ACC_BIAS3_X]*dImuData.dTemp3);
	b = +(in[1] + p[IMU_ACC_BIAS_Y] + p[IMU_ACC_BIAS1_Y]*dImuData.dTemp + p[IMU_ACC_BIAS2_Y]*dImuData.dTemp2 +
		  p[IMU_ACC_BIAS3_X]*dImuData.dTemp3);
	c = -(in[2] + p[IMU_ACC_BIAS_Z] + p[IMU_ACC_BIAS1_Z]*dImuData.dTemp + p[IMU_ACC_BIAS2_Z]*dImuData.dTemp2 +
		  p[IMU_ACC_BIAS3_X]*dImuData.dTemp3);
...
void mpu6000PreInit(void) {
	mpu6000Data.spi = spiClientInit(DIMU_MPU6000_SPI, MPU6000_SPI_REG_BAUD, 0, DIMU_MPU6000_CS_PORT, DIMU_MPU6000_CS_PIN, &mpu6000Data.spiFlag,
									0);
}
...
void mpu6000Init(void) {
	...
	// LPF
	if (p[IMU_MPU_LPF] > 2100.0 || p[IMU_MPU_LPF] == 0.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_2100HZ_NOLPF);
	} else if (p[IMU_MPU_LPF] > 188.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_256HZ);
	} else if (p[IMU_MPU_LPF] > 98.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_188HZ);
	} else if (p[IMU_MPU_LPF] > 42.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_98HZ);
	} else if (p[IMU_MPU_LPF] > 20.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_42HZ);
	} else if (p[IMU_MPU_LPF] > 10.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_20HZ);
	} else if (p[IMU_MPU_LPF] > 5.0) {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_10HZ);
	} else {
		mpu6000ReliablySetReg(26, BITS_DLPF_CFG_5HZ);
	}
