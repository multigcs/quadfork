...
void gimbalInit(void) {
	...
	gimbalData.roiLat = (double)0.0f;
	gimbalData.roiLon = (double)0.0f;
	gimbalData.roiAlt = (double)0.0f;
	...
	chkTim = ((int)p[GMBL_PWM_FREQ] != p[MOT_PWM_FREQ]);
...
void gimbalUpdate(void) {
	...
	if (p[GMBL_TILT_PORT]) {
		tilt = RADIO_AUX3 + gimbalData.roiAngle * GMBL_DEGREES_TO_PW * p[GMBL_SCAL_PITCH];
		// smooth
